      ...
      {Object.entries(feedbackTypes).map(([key, item]) => {
        return (
          ...
          onClick={() => sendFeedbackType(key as FeedbackType)}
            key={key}
            type="button"
            className="bg-zinc-800 rounded-lg py-5 w-24 flex-1 flex flex-col items-center gap-2 border-2 border-transparent hover:border-brand-400 hover:animate-pulse focus:border-brand-400 focus:outline-none"
          >
